...
import { parseFlow, elementFlow, elementSink, pipe, ValueChunk, HeaderPart, FragmentsPart, PreamblePart } from 'https://esm.sh/gh/jpambrun/dicom-streams-js@f250f9f'
...
class FilterLargeValue extends stream.Transform {
    ...
    _transform(chunk, encoding, callback) {
        if (chunk instanceof ValueChunk && chunk?.bytes?.length > this.maxSize && this.VRToFilter.includes(this.currentVR)) {
...
const convertToDicomweb = (src, dst = {}) => {
    ...
    for (const element of data) {
        ...
        if (value) {
            switch (vr.name) {
                ...
                case "OF":
                    if (element.value.bytes.length > 0) {
                        ...
                        dst[hextag] = { vr: vr.name, BulkDataURI: `?range=${element.range[0]}-${element.range[0] + element.range[1]}` };
                    } else {
            ...
            dst[hextag] = {
                vr: vr.name, value: fragments.map(f => {
    ...
    const stream = pipe(
        ...
        elementSink(async elements => {
            try {
                ...
                const patientID = dicomDict["00100020"]?.Value?.[0] || '';
                ...
                const studyDate = dicomDict["00080020"]?.Value?.[0] || '';
                const studyDescription = dicomDict["00081030"]?.Value?.[0] || '';
