...
router.get("/dicomweb/studies", async (ctx) => {
    ...
    const studies = [...db.getRange({ start: "study", end: "study" + "\ufffd" }).filter(({ value: dicomDict }) => {
    ...
    for (const study of studies) {
        ...
        study["00201208"] = { vr: "IS", Value: [await db.getCount({ start: `instance:${studyInstanceUID}`, end: `instance:${studyInstanceUID}\ufffd` })] };
...
router.get("/dicomweb/studies/:studyInstanceUID/series", async (ctx) => {
    ...
    const series = [...db.getRange({ start: `series:${studyInstanceUID}:`, end: `series:${studyInstanceUID}:\ufffd` }).map(({ value }) => value)];

    for (const s of series) {
        const seriesInstanceUID = s["0020000E"]?.Value?.[0];
        s["00201209"] = { vr: "IS", Value: [await db.getCount({ start: `instance:${studyInstanceUID}:${seriesInstanceUID}`, end: `instance:${studyInstanceUID}:${seriesInstanceUID}\ufffd` })] };
    }
...
router.get("/dicomweb/studies/:studyInstanceUID/series/:seriesInstanceUID/metadata", async (ctx) => {
    ...
    const instances = [...db.getRange({ start: `instance:${studyInstanceUID}:${seriesInstanceUID}:`, end: `instance:${studyInstanceUID}:${seriesInstanceUID}:\ufffd` }).map(({ value }) => value)];
    if (!instances.length) {
...
router.get("/dicomweb/studies/:studyInstanceUID/series/:seriesInstanceUID/instances/:sopInstanceUID/frames/:frameNumber", async (ctx) => {
    ...
    if (dicomDict["00280008"]?.Value?.[0] > 1) {
        ...
        if (!samplesPerPixel || !rows || !cols || !bitsAllocated) throw new Error(`Missing required DICOM attributes for multiframe data ${samplesPerPixel}, ${rows}, ${cols}, ${bitsAllocated}`);
        ...
        const frameStream = fs.createReadStream(filePath, { start: start + frameIndex * bytesPerFrame, end: start + (frameIndex + 1) * bytesPerFrame, highWaterMark: 1024 * 1024 });
